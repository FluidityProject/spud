...
Returns 0 if the specifed key does not exist in the options tree.
...
exist, creates the option (with data type \lstinline+SPUD_TYPE_NONE+) and
...
As \ref{sec:set_option}, but additional attempts to mark the option at the
specified key as an attribute.
...
\item If \lstinline+key+ fails to match, creates a new option at the supplied
  key, sets the option to \lstinline+val+ and returns error code
  \lstinline+SPUD_NEW_KEY_WARNING+.
\item If \lstinline+key+ matches an option, but the option has children, sets
  the option to \lstinline+val+ and returns error code
