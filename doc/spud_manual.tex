...
libspud.c is a module written in C using the header file Python.h and spud.h.
It provides a python interface to libspud; so that users could use python codes to access the C codes in libspud.
The module takes in python arguments, converts them into C arguments and then call the corresponding C functions with the converted C arguments. 

\subsection{Errors}

The module inherits all the errors from libspud. Here is the list of them: \\* 
SpudError; \\* 
SpudTypeError; \\* 
SpudKeyError; \\* 
SpudFileError; \\* 
...
\begin{tabular}{lll}
  ...
  \lstinline+SPUD_REAL+ & \lstinline+SPUD_DOUBLE+ & \lstinline+PyFloat_Type+ \\
  \lstinline+SPUD_INTEGER+ & \lstinline+SPUD_INT+ & \lstinline+PyInt_Type+ \\
  \lstinline+SPUD_NONE+ & \lstinline+SPUD_NONE+ & \lstinline+Py_RETURN_NONE+ \\
  \lstinline+SPUD_CHARACTER+ & \lstinline+SPUD_STRING+ & \lstinline+PyString_Type+
\end{tabular}
...
This function takes no arguements and returns None.
Clears the entire options tree.
