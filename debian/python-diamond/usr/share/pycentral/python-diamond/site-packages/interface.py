...
Important fields:
  ...
  fl_data_paths: XML paths to Fluidity data
...
class Diamond:
  def __init__(self, gladefile, schemafile = None, logofile = None, input_filename = None, fl_dim_path = "/fluidity_options/geometry/dimension"):
    self.gladefile = gladefile
    self.gui = gtk.glade.XML(self.gladefile)

    self.statusbar = DiamondStatusBar(self.gui.get_widget("statusBar"))
    self.menubar   = DiamondMenuBar(self, logofile)
    ...
    signals     =  {"on_new": self.on_new,
                    ...
                    "on_visualise": self.menubar.on_visualise,
                    "on_expand_all": self.menubar.on_expand_all,
                    "on_collapse_all": self.menubar.on_collapse_all,
                    "on_find": self.find.on_find,
                    "on_about": self.menubar.on_about}
    ...
    if logofile is not None:
      gtk.window_set_default_icon_from_file(logofile)

    self.init_treemodel()

    # FLUIDITY
    self.fl_data_paths = {}
    self.fl_data_paths["dim"] = fl_dim_path
  ...
  def set_saved(self, saved, filename = ""):
    ...
    if not filename == "":
      ...
      if not filename is None:
        self.file_path = filename[:len(filename) - len(filename.split("/")[len(filename.split("/")) - 1])]
    self.update_title()
    ...
    if not schemafile == "" and not schemafile == self.schemafile:
      if schemafile is None:
        ...
        try:
          ...
          return
        self.schemafile_path = schemafile[:len(schemafile) - len(schemafile.split("/")[len(schemafile.split("/")) - 1])]
  ...
  def on_open(self, widget=None):
    ...
    for xmlname in config.schemata:
      filter_names_and_patterns[config.schemata[xmlname]] = "*." + xmlname
    file_selection = dialogs.get_filename(title = "Open XML file", action = gtk.FILE_CHOOSER_ACTION_OPEN, filter_names_and_patterns = filter_names_and_patterns, folder_uri = self.file_path)

    if not file_selection is None:
      self.open_file(filename = file_selection[0])
  ...
  def on_open_schema(self, widget=None):
    ...
    file_selection = dialogs.get_filename(title = "Open RELAX NG schema", action = gtk.FILE_CHOOSER_ACTION_OPEN, filter_names_and_patterns = {"RNG files":"*.rng"}, folder_uri = self.schemafile_path)
    if not file_selection is None:
      self.open_file(schemafile = file_selection[0])
  ...
  def on_save_as(self, widget=None):
    ...
    for xmlname in config.schemata:
      filter_names_and_patterns[config.schemata[xmlname]] = "*." + xmlname
    file_selection = dialogs.get_filename(title = "Save XML file", action = gtk.FILE_CHOOSER_ACTION_SAVE, filter_names_and_patterns = filter_names_and_patterns, folder_uri = self.file_path)

    if not file_selection is None:
      filename = file_selection[0]
      try:
        file_extension = filter_names_and_patterns[file_selection[1]][1:]
        if len(filename) < len(file_extension) or not filename[len(filename) - len(file_extension):] == file_extension:
  ...
  def set_geometry_dim_tree(self):
    ...
    iter = self.get_treestore_iter_from_xmlpath(self.fl_data_paths["dim"])
    if iter is None:
...
class DiamondMenuBar:
  def __init__(self, parent, logofile):
    self.parent = parent
  ...
  def on_expand_all(self, widget=None):
    ...
    self.parent.treeview.expand_all()
  ...
  def on_collapse_all(self, widget=None):
    ...
    self.parent.treeview.collapse_all()
  ...
  def on_visualise(self, widget):
    ...
    if ret != 0:
      dialogs.error(self.parent.main_window, "popstate binary not available. Have you installed fluidity tools?")
      return
    ...
    for i in range(len(possible_mayavi_cmds)):
      ...
      if ret == 0:
        ...
        dialogs.error(self.parent.main_window, "mayavi not available. Have you installed mayavi?")
        return

    tmp = tempfile.NamedTemporaryFile()
    name = tmp.name
    self.parent.tree.write(name + ".xml")

    std_input, std_output, err_output = os.popen3("popstate %s %s" % (name + ".xml", name))
    output = err_output.read()
    if len(output) > 0:
      cwd = os.getcwd()
      os.chdir(self.parent.file_path)
      std_input, std_output, err_output = os.popen3("popstate %s %s" % (name + ".xml", name))
      os.chdir(cwd)
      output = err_output.read()
      if len(output) > 0:
        dialogs.long_message(self.parent.main_window, output + "\npopstate enocuntered an error.\n")
        return
